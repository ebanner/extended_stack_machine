...
void do_args (int argc, char** argv)
{
    ...
    obj = &cin;			/* input file is initially stdin	 */
    interact = &cin;		/* interactive input is too		 */
    progname = *argv++;
    cp = strrchr(progname, '/');
    if (cp != NULL)
	progname = cp+1;
...
void pscan()
{
	char c;
    	...
    	obj->get(c);
	if (!obj->good() || c != '%') {
	    cerr << "Illegal object module format (pscan)" << endl;
	    exit(1);
	}
        skip_to_eoln();
}
...
//  read the header information
	...
	cerr << "Illegal object module format (initialize)" << endl;
	exit(1);
      }
...
#ifdef DEBUG
    cerr << "finished scanning header..." << endl;
#endif
    pscan();
...
void trace (int ppc, const char* cp)
{
    int     i,
